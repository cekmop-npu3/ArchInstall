...
[[ -n "${ROOT_DIR:-}" ]] || echo "ROOT_DIR env variable is not set" && return $IP_ROOT_DIR_INVALID

[[ -e "$ROOT_DIR/scripts/utils/parse_options.sh" ]] || echo "ROOT_DIR is invalid" && return $IP_ROOT_DIR_INVALID
